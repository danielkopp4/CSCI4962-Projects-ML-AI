...
forever_deck = list(pydealer.Deck())
order = np.arange(len(forever_deck))

def random_card(size=1):
    choices = np.random.choice(order, size=size, replace=True)
    return [forever_deck[choice] for choice in choices]
...
class BlackJackGame:
    ...
    def reset_round(self):
        ...
        if not self.infinite:
            ...
            self.dealer = set(random_card(2))

        # give player cards
        if not self.infinite:
            self.player = set(self.deck.deal(2))
        else:
            self.player = set(random_card(2))
    ...
    def stand(self):
        ...
        # print(f"ds {dealer_sum} ps {self.player_sum}")
        if dealer_sum == self.player_sum:
            return_val = PUSH
        elif dealer_sum > 21 or self.player_sum > dealer_sum:
            return_val = WIN
        else:
            return_val = LOSE

        self.last_state = return_val
    ...
    def ended(self):
        return (
            self.last_state == WIN or self.last_state == LOSE
        ) and (
            self.infinite or len(self.deck) < 0.5 * 52
        )
